...
function Overview({ data, accessToken, openDetail }: { data: ReportData; accessToken: string; openDetail: (key: DetailKey, title: string, description: string) => void }) {
  ...
  return <div className="space-y-8">
    ...
    <footer className="text-xs text-muted-foreground">Canonical filtered report · {report.raw.sessions} raw sessions · {report.filteredAutomationSessions} suspected automated sessions excluded · {report.filteredInternalSessions} internal/test sessions excluded from headline audience counts. Legitimate PDF and download actions remain protected by the canonical intent rules.</footer>
  </div>;
}
...
export default function AdminAnalyticsReport({ accessToken }: { accessToken: string }) {
  ...
  return <><div className="flex flex-col gap-4 border-y border-border py-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-1 overflow-x-auto pb-1">{RANGES.map((item) => <Button key={item.value} size="sm" variant={range === item.value ? "default" : "ghost"} onClick={() => setRange(item.value)}>{item.label}</Button>)}</div><Button variant="outline" size="sm" onClick={() => void load()} disabled={loading}><RefreshCw className={loading ? "animate-spin" : ""} />{loading ? "Loading…" : "Refresh"}</Button></div>{error && <p className="mt-5 text-sm text-destructive">{error}</p>}{!data && !error ? <p className="py-12 text-center text-muted-foreground">Preparing your report…</p> : data && <Tabs defaultValue="owner" className="mt-6"><TabsList className="h-auto w-full justify-start overflow-x-auto rounded-md bg-muted/60 p-1"><TabsTrigger value="owner">Owner Summary</TabsTrigger><TabsTrigger value="overview">Overview</TabsTrigger><TabsTrigger value="publications">Publications</TabsTrigger><TabsTrigger value="reach">Reach</TabsTrigger><TabsTrigger value="visitors">Visitors</TabsTrigger><TabsTrigger value="settings">Settings</TabsTrigger></TabsList><TabsContent value="owner" className="mt-8"><OwnerSummary data={data} accessToken={accessToken} openDetail={(key, title, description) => setDetail({ key, title, description })} /></TabsContent><TabsContent value="overview" className="mt-8"><Overview data={data} accessToken={accessToken} openDetail={(key, title, description) => setDetail({ key, title, description })} /></TabsContent><TabsContent value="publications" className="mt-8"><Publications data={data} /></TabsContent><TabsContent value="reach" className="mt-8"><Reach data={data} accessToken={accessToken} /></TabsContent><TabsContent value="visitors" className="mt-8"><VisitorActivitySection accessToken={accessToken} embedded /></TabsContent><TabsContent value="settings" className="mt-8"><Settings accessToken={accessToken} /></TabsContent></Tabs>}
