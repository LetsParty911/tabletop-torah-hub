  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h3 className="font-serif text-lg font-semibold">Download actions by minute</h3>
          <p className="text-xs text-muted-foreground">
            {total.toLocaleString()} raw actions · peak {peak.toLocaleString()}/min · times shown ET
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-1">
          {WINDOWS.map((w) => (
            <button
              ...
              className={`rounded-full px-3 py-1.5 text-sm ${minutes === w.minutes ? "bg-primary text-primary-foreground" : "border border-border"}`}
            >
              {w.label}
            </button>
          ))}
          <button
            ...
            className="ml-1 rounded-full border border-border px-3 py-1.5"
          >
            {loading ? (
        ...
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={series} margin={{ top: 8, right: 8, bottom: 0, left: -20 }}>
            ...
            <Tooltip
              ...
              labelFormatter={(v) => `${formatTick(String(v))} ET`}
              formatter={(v) => [String(v), "Download actions"]}
            />
