...
async function fetchRows(since: string): Promise<Row[]> {
  ...
  for (let offset = 0; offset < 100000; offset += pageSize) {
    const { data, error } = await admin.from("analytics_events")
      .select("event_name, occurred_at, visitor_id, session_id, is_new_visitor, publication_id, publication_title, publication_series, publisher, parsha, device_type, source_group, metadata")
      .gte("occurred_at", since).order("occurred_at", { ascending: true }).range(offset, offset + pageSize - 1);
...
async function priorVisitors(visitorIds: string[], since: string): Promise<Set<string>> {
  ...
  for (let i = 0; i < visitorIds.length; i += 100) {
    ...
    const { data, error } = await admin.from("analytics_events")
      .select("visitor_id").in("visitor_id", batch).lt("occurred_at", since).limit(10000);
    if (error) throw new Error(error.message);
...
export const adminPhase1DashboardV2 = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const visitorIds = [...new Set(rows.map((r) => s(r.visitor_id)).filter((v): v is string => Boolean(v)))];
    const prior = await priorVisitors(visitorIds, since);

    const sessions = new Map<string, Session>();
    type Pub = { id: string; title: string; parsha: string | null; series: string | null; publisher: string | null; impressions: Set<string>; clicks: Set<string>; accesses: Set<string>; downloads: Set<string> };
    ...
    for (const row of rows) {
      ...
      if (!session) {
        session = {
          ...
          impressions: new Set(), clicks: new Set(), accesses: new Set(), downloads: new Set(),
          downloadActions: 0, activeSeconds: 0,
        };
      ...
      if (["publication_click", "pdf_open", "download", "filter_change", "search", "share_click", "signup"].includes(name)) session.positiveAction = true;
      const pubId = s(row.publication_id);
      if (!pubId) continue;
      const pair = `${sid}::${pubId}`;
      let pub = pubs.get(pubId);
      if (!pub) {
        pub = { id: pubId, title: s(row.publication_title) ?? "Untitled", parsha: s(row.parsha), series: s(row.publication_series), publisher: s(row.publisher), impressions: new Set(), clicks: new Set(), accesses: new Set(), downloads: new Set() };
        pubs.set(pubId, pub);
      }
      if (name === "publication_impression") { session.impressions.add(pair); pub.impressions.add(pair); }
      if (name === "publication_click") { session.clicks.add(pair); pub.clicks.add(pair); }
      if (ACCESS.has(name)) { session.accesses.add(pair); pub.accesses.add(pair); }
      if (name === "download") { session.downloads.add(pair); pub.downloads.add(pair); session.downloadActions += 1; }
    }

    const all = [...sessions.values()];
    const uniqueVisitors = new Set(all.map((x) => x.visitorId).filter((v): v is string => Boolean(v)));
    ...
    const engaged = all.filter((x) => x.positiveAction || x.heartbeat || x.pageViews >= 2 || x.activeSeconds >= 10);
    const lowConfidence = all.filter((x) => !x.positiveAction && !x.heartbeat && x.pageViews <= 1 && x.activeSeconds < 10);
    ...
    const group = (key: (x: Session) => string) => {
      const map = new Map<string, { key: string; sessions: number; clicks: number; accesses: number; uniquePdfDownloads: number; downloadActions: number; convertedSessions: number }>();
      for (const x of all) {
        const k = key(x);
        const a = map.get(k) ?? { key: k, sessions: 0, clicks: 0, accesses: 0, uniquePdfDownloads: 0, downloadActions: 0, convertedSessions: 0 };
        a.sessions += 1;
    ...
    const publications = [...pubs.values()].map((p) => ({
      id: p.id, title: p.title, parsha: p.parsha, series: p.series, publisher: p.publisher,
      impressions: p.impressions.size, clicks: p.clicks.size, accesses: p.accesses.size, downloads: p.downloads.size,
    })).sort((a, b) => b.downloads - a.downloads || b.accesses - a.accesses).slice(0, 50);
